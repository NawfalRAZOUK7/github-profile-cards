import type { UserStats } from '../metrics/overview.js';
import type { OverviewVariant, OverviewRenderContext } from './types.js';
...
function toContext(stats: UserStats, avatarHref?: string): OverviewRenderContext {
  return {
    stats: {
      name: stats.name,
      login: stats.login,
...
export function renderOverviewSvg(
  stats: UserStats,
  avatarHref: string | undefined,
  variant: OverviewVariant = 1,
): string {
